...
export function createPeerConnection(peerId, localStream, socket, currentRoomId) {
...
export async function handleOffer(data, socket, currentRoomId) {
    const peerId = data.peerId;
    const pc = peerConnections.get(peerId) || createPeerConnection(peerId, localStream, socket, currentRoomId);
...
export async function startCallForRoom(myPeerId, peerList, socket, currentRoomId) {
    if (!myPeerId) return;
    ...
    if (remotePeerId && !peerConnections.has(remotePeerId)) {
        ...
        setTimeout(async () => {
            const pc = createPeerConnection(remotePeerId, localStream, socket, currentRoomId);
